...
class ReinstallCommand implements ICommand {
  ...
  function reinstall(library : LibraryConfig, force : Bool) : Void {
    return switch library {
      case Haxelib(name, version, skipDependencies): reinstallHaxelib(name, version, skipDependencies, force);
      case Git(name, url, skipDependencies, ref, dir): reinstallGit(name, url, skipDependencies, ref, dir, force);
      case Mercurial(name, url, skipDependencies, ref, dir): reinstallMercurial(name, url, skipDependencies, ref, dir, force);
      case Dev(name, path, skipDependencies) : reinstallDev(name, path, skipDependencies, force);
    };
  }

  function reinstallHaxelib(name : String, version : Option<String>, skipDependencies : Option<Bool>, force : Bool) {
    if (force || !Shell.isAlreadyInstalledHaxelib(name, version, { log: false, throwError: true })) {
      ...
      Shell.haxelibInstall(name, version, skipDependencies, { log: true, throwError: false });
    } else {
      Log.info('haxelib library "$name" (version: ${version.getOrElse('N/A')}) already installed');
    }
  }

  function reinstallGit(name : String, url : String, skipDependencies : Option<Bool>, ref : Option<String>, dir : Option<String>, force: Bool) {
  ...
  function reinstallMercurial(name : String, url : String, skipDependencies : Option<Bool>, ref : Option<String>, dir : Option<String>, force: Bool) {
  ...
  function reinstallDev(name : String, path : String, skipDependencies : Option<Bool>, force: Bool) : Void {
